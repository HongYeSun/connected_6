...
const Feed = ({ onSelectVideo }) => {
    ...
    const [GenderTopVideos, setGenderTopVideos] = useState([]);
    ...
    useEffect(() => {
        const fetchTopVideos = async () => {
            try {
                ...
                const {total, gender} = response.data;


                setTopVideos(total);
                setGenderTopVideos(gender);
                setLoading(false);
            } catch (error) {
    ...
    return (
        ...
        <Scroller direction="vertical">
                ...
                <h2><Icon>star</Icon>Your Gender Picked Top 10 Videos</h2>
                <div className={css.videoLine}>
                    {GenderTopVideos.map((video, index) => (
                        <div key={index} className={css.videoContainer}>
                            <div className={css.ranking}>{index + 1}</div>
                            <ImageItem
                                inline
